...
#' @export
basicKineticModel <- function(times=NA_real_,synthRate=NA_real_,degRate=NA_real_,initAbund=NA_real_,ids=NA,data=NULL,spikeIns=NULL,expMetadata=data.frame(),normFactors=NA_real_,dispersionModel=function(x){}){
  ...
  model=new("basicKineticModel",times=times,synthRates=synthRate,degRates=degRate,initVals=initAbund,ids=ids,data=data,spikeIns=spikeIns,
            expMetadata=expMetadata,normFactors=normFactors,dispersionModel=dispersionModel)
  if(is.na(model@normFactors) & nrow(model@data)!=0)
  {
